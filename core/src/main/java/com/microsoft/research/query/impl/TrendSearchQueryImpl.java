...
/**
 * The Class TrendSearchQueryImpl.
 */
public class TrendSearchQueryImpl extends
		BaseAcademicSearchApiQuery<TrendPoint> implements TrendSearchQuery {

	/**
	 * Instantiates a new trend search query impl.
	 * 
	 * @param applicationId the application id
	 */
	public TrendSearchQueryImpl(String applicationId) {
		super(applicationId);
	}
	...
	public void reset() {
		...
		apiUrlBuilder.withParameter(ParameterNames.RESULT_OBJECTS,
				ObjectType.PUBLICATION_TREND.value());
	}
	 ...
	 */
	@Override
	protected PagedList<TrendPoint> unmarshallList(JsonObject response) {
		JsonObject data = response.get("Trend").getAsJsonObject();
		PagedArrayList<TrendPoint> list = new PagedArrayList<TrendPoint>();
		if (data != null) {
			Gson gson = getGsonBuilder().create();
			List<TrendPoint> resultsList = gson.fromJson(data.get("Trend"),
					new TypeToken<List<TrendPoint>>() {
					}.getType());
