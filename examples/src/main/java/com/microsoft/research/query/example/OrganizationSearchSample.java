...
/**
 * The Class OrganizationSearchSample.
 */
public class OrganizationSearchSample {
    ...
    private static void processCommandLine(CommandLine line, Options options) {
        if(line.hasOption(HELP_OPTION)) {
    		...
    		OrganizationSearchQuery query = factory.newOrganizationSearchQuery();
    		PagedList<Organization> response = query.withFullTextQuery(line.getOptionValue(QUERY_OPTION)).withStartIndex(1).withEndIndex(5).list();
	 ...
	 */
	private static void printResponse(PagedList<Organization> response) {
		...
		for (Organization result : response) {
    ...
    private static void printHelp(Options options) {
        int width = 80;
        String syntax = OrganizationSearchSample.class.getName() + " <options>";
