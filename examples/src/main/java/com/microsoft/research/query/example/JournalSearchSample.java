...
/**
 * The Class JournalSearchSample.
 */
public class JournalSearchSample {
    ...
    private static void processCommandLine(CommandLine line, Options options) {
        if(line.hasOption(HELP_OPTION)) {
    		...
    		JournalSearchQuery query = factory.newJournalSearchQuery();
    		PagedList<Journal> response = query.withFullTextQuery(line.getOptionValue(QUERY_OPTION)).withStartIndex(1).withEndIndex(5).list();
	 ...
	 */
	private static void printResponse(PagedList<Journal> response) {
    ...
    private static void printHelp(Options options) {
        int width = 80;
        String syntax = JournalSearchSample.class.getName() + " <options>";
